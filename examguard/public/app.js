...
async function startExam(s) {
  ...
  s.timerHandle = setInterval(() => {
    ...
    const sidebarEl = el('#examSidebar');
    if (sidebarEl) {
      const totalSec = s.exam.durationMinutes * 60;
      const frac = s.remainingSec / totalSec;
      sidebarEl.classList.toggle('fire', frac <= 0.1);
      sidebarEl.classList.toggle('ambar', frac > 0.1 && frac <= 0.25);
    }
...
// Reflects the current warning count in the sidebar, turning it red once
// the next violation would terminate the attempt — same visual cue as the
// warning pill in the reference design.
function updateWarnDisplay(s) {
  const val = el('#warnVal');
  const row = el('#warnCountRow');
  if (!val || !row || !s.attempt) return;
  val.textContent = s.attempt.warnings;
  const limit = s.exam.warningLimit;
  const hot = limit != null && s.attempt.warnings > 0 && s.attempt.warnings >= limit - 1;
...
function renderExam(s) {
  renderShell('student', h`
    <div class="exam-shell">
      ...
      <div class="exam-sidebar panel" id="examSidebar">
        <div class="small">Time remaining</div>
        <div class="timer" id="timerVal">${fmtClock(s.remainingSec)}</div>
        <div class="warn-count" id="warnCountRow">Warnings: <span id="warnVal">${s.attempt.warnings}</span>/${s.exam.warningLimit ?? '∞'}</div>
        <div class="divider"></div>
